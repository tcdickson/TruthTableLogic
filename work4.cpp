...
bool primaryExpression(istream& input, bool get) {
  ...
  switch (currTok) {
    ...
    case LP:
      ...
      break;
    case DISPLAY: {
      cout << "Evaluating display variable: " << stringValue << " Current Token: " << currTok << endl;
      if (table.find(stringValue) == table.end()) {
        return error("Unknown variable");
      }
      bool value = table[stringValue];
      getToken(input); // Advance to next token after handling variable
      return value;
    }
    default:
...
bool orEvaluation(istream& input, bool get) {
  ...
  bool left = andEvaluation(input, get); // Evaluate the left side (which might include AND operations)
  ...
  cout << "About to perform OR operation, left value: " << left << endl;    // Debug print current 
    getToken(input); // Move to the next token before evaluating the right operand
    left = left || andEvaluation(input, true); // Evaluate the OR operation with the next term
  }
    ...
    void generateForTwoVariables() {
        ...
        for (const auto& combo : possibleCombinations) {
            ...
            table["B"] = combo[1];
                        ...
                        cout << "Evaluating with A=" << table["A"] << ", B=" << table["B"] << endl;
